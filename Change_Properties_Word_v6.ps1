...
class Word {
    ...
    [void]GetDocumentProperties() {
        $properties = @{}
        $propertyNames = @("Author", "Title", "Subject", "Keywords", "Category", "Comments", "RevisionNumber", "Version")

        foreach ($propertyName in $propertyNames) {
            try {
                $property = $this.Document.BuiltInDocumentProperties.Item($propertyName)
                if ($null -ne $property) {
                ...
                Write-Warning "Failed to get property '$propertyName': $_"
                $properties[$propertyName] = $null
            }
        }
