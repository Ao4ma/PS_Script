class WordDocument {
    ...
    WordDocument([string]$docFileName, [string]$docFilePath, [string]$scriptRoot) {
        ...
        $this.WordApp.DisplayAlerts = [Microsoft.Office.Interop.Word.WdAlertLevel]::wdAlertsNone        $docPath = Join-Path -Path $this.DocFilePath -ChildPath $this.DocFileName
...
[void] Delete_Property([string]$propName) {
 ...
 [void] FillSignatures() {
    ...
    $cell3.Range.ParagraphFormat.Alignment = [Microsoft.Office.Interop.Word.WdParagraphAlignment]::wdAlignParagraphCenter
