...
<#
if (-not $docFilePath) {
    Write-Error "Usage: .\ImportModules4.ps1 -docFileName <path to doc file>"
    exit 1
}
...
# デフォルトのドキュメントファイルパスを設定
$defaultDocFilePath = Join-Path -Path $scriptRoot -ChildPath "技100-999.docx"

# 引数が指定されていない場合はデフォルトのファイルパスを使用
if (-not $docFilePath) {
...
if (-not (Test-Path $docFilePath)) {
    Write-Error "File not found: $docFilePath"
    exit 1
}
...
$propValue2 = $wordDoc.Read_Property2("CustomProperty2")
Write-Host "Read Property Value: $propValue2"
$propValue3 = $wordDoc.Read_Property2("CustomProperty31")
Write-Host "Read Property Value: $propValue3"
