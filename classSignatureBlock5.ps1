...
try {
    ...
    write-host "アセンブリが $assemblyPath から正常に読み込まれました"
} catch {
...
function Find-SignatureTable {
    ...
    for ($i = 1; $i -le $doc.Tables.Count; $i++) {
        ...
        if ($table -is [Microsoft.Office.Interop.Word.Table]) {
            ...
            write-host "Table Index: $($i), Top-Left Position: $($topLeft), Bottom-Right Position: $($bottomRight), Rows: $($table.Rows.Count), Columns: $($table.Columns.Count)"
        } else {
            write-host "Table Index: $($i) is not a valid Table object."
        }
    }
...
# $doc = $word.Documents.Open("D:\GitHub\PS_Script\技100-999.docx")
 ...
 $cell_Info | ForEach-Object { write-host "セル番号: $($_.Cell_Number), 位置: $($_.Position), 座標: $($_.Position)" }
    
 # 役割上セルセットの情報を取得
 $role_Above_Cell_Set = $signature_Block.Get_Role_Above_Cell_Set()
 $role_Above_Cell_Set | ForEach-Object { write-host "役割: $($_.Role), 役割セル: $($_.Role_Cell), 名前セル: $($_.Name_Cell)" }
    
 # 役割左セルセットの情報を取得
 $role_Left_Cell_Set = $signature_Block.Get_Role_Left_Cell_Set()
 $role_Left_Cell_Set | ForEach-Object { write-host "役割: $($_.Role), 役割セル: $($_.Role_Cell), 名前セル: $($_.Name_Cell)" }
