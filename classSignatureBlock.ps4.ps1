...
$assemblyPath = "C:\Program Files (x86)\Microsoft Office\root\Office16\Microsoft.Office.Interop.Word.dll"
try {
    Add-Type -Path $assemblyPath -ErrorAction Stop
} catch {
    Write-Error "Failed to load assembly from $assemblyPath. Error: $_"
    exit 1
}
...
function Find-SignatureTable {
    ...
    $wdInformation = [enum]::GetValues([Microsoft.Office.Interop.Word.WdInformation])
    $highest_Table = $null
    ...
    for ($i = 1; $i -le $doc.Tables.Count; $i++) {
        ...
        if ($table -is [Microsoft.Office.Interop.Word.Table]) {
            $topLeft = $table.Cell(1, 1).Range.Information($wdInformation::wdVerticalPositionRelativeToPage)
            $bottomRight = $table.Cell($table.Rows.Count, $table.Columns.Count).Range.Information($wdInformation::wdVerticalPositionRelativeToPage)
...
class Signature_Block {
    ...
    Signature_Block($doc, [string[]]$roles) {
        ...
        Write-Output "Signature Table Object Type: $($this.Table.GetType().FullName)"
        $this.Validate_Roles()
    }
