...
# -replace '\[', '[[]':
# 
# -replace演算子は、文字列の置換を行います。
# '\['は正規表現で、リテラルの[を意味します。\はエスケープ文字で、[を特別な意味を持たないリテラル文字として扱います。
# '[[]'は、置換後の文字列です。[をエスケープするために、[を二重にしています。PowerShellでは、[をエスケープするために[[]と書きます。
# -replace '\]', '[]]':
# 
# -replace演算子は、文字列の置換を行います。
# '\]'は正規表現で、リテラルの]を意味します。\はエスケープ文字で、]を特別な意味を持たないリテラル文字として扱います。
# '[]]'は、置換後の文字列です。]をエスケープするために、[と]を組み合わせて[]]と書きます。
# 
# 
