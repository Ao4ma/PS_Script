...
function New-ConversionLists {
    foreach ($sourceFolder in $sourceFolders) {
        $fullSourcePath = Join-Path -Path $commonPath -ChildPath $sourceFolder
        $conversionList = New-ConversionList -sourceFolder $fullSourcePath
        Save-ConversionList -sourceFolder $fullSourcePath -conversionList $conversionList -fileIndex 1
    }
}

# 処理の中断と再開
function Invoke-FoldersProcessing {
    param (
        [string]$mode,
        [int]$limit
    )
    ...
    if ($mode -eq "specific" -and $specificFolder -ne "") {
        $fullSpecificPath = Join-Path -Path $commonPath -ChildPath $specificFolder
        while (-not $stopProcessing) {
            if ($initialState) {
                Get-ConversionList -sourceFolder $fullSpecificPath
                $initialState = $false
            }
            Convert-Folder -sourceFolder $fullSpecificPath -mode $mode -limit $limit
        }
    } else {
        foreach ($sourceFolder in $sourceFolders) {
            $fullSourcePath = Join-Path -Path $commonPath -ChildPath $sourceFolder
            while (-not $stopProcessing) {
                if ($initialState) {
                    Get-ConversionList -sourceFolder $fullSourcePath
                    $initialState = $false
                }
                Convert-Folder -sourceFolder $fullSourcePath -mode $mode -limit $limit
            }
        }
    }
}
...
Invoke-FoldersProcessing -mode $mode -limit $limit
