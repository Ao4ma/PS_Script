...
class Word {
    ...
    [void]GetDocumentProperties() {
        $properties = @{
            DocumentTheme        = $this.Document.DocumentTheme
            HasVBProject         = $this.Document.HasVBProject
            OMathFontName        = $this.Document.OMathFontName
            EncryptionProvider   = $this.Document.EncryptionProvider
            UseMathDefaults      = $this.Document.UseMathDefaults
            CurrentRsid          = $this.Document.CurrentRsid
            DocID                = $this.Document.DocID
            CompatibilityMode    = $this.Document.CompatibilityMode
            CoAuthoring          = $this.Document.CoAuthoring
            Broadcast            = $this.Document.Broadcast
            ChartDataPointTrack  = $this.Document.ChartDataPointTrack
            IsInAutosave         = $this.Document.IsInAutosave
            WorkIdentity         = $this.Document.WorkIdentity
            AutoSaveOn           = $this.Document.AutoSaveOn
        }
    ...
    [void]SetDocumentProperties([hashtable]$properties) {
        foreach ($key in $properties.Keys) {
            try {
                $this.Document.PSObject.Properties[$key].Value = $properties[$key]
